 ...
 * @param {*} record
 */
export function customToJSON(Modal, record) {
  return Object.entries(record).reduce(
    (acc, [key, value]) => {
      if (key === 'id' || key === '_id') {
        return acc;
      }
      if (Modal.definition.serializationOmitAttributes.includes(key)) {
        return acc;
      }
      // if (key.startsWith('_')) {
...
export function validate(Model, data, strictMode = false) {
  const values = {};
  const issues = [];
  ...
  data = Object.entries(data).reduce(
    (acc, [field, value]) =>
      field.startsWith('_') || Model.definition.validationOmitAttributes.includes(field)
        ? { ...acc }
        : { ...acc, [field]: value },
    {},
  );
 ...
 * @param {string} field
 */
export function association(field) {
  ...
  let throughCollection =
    collections[config.through] ||
  ...
  if (!throughCollection && isReflexiveAssociation) {
    throughCollection =
      collections[`${collection.identity}_${field}__${collection.identity}_${field}_${collection.identity}`];
  }
...
export async function lookupByAssociationWithId(field, criterion) {
